...
router.get('/', async (_req, res, next) => {
  try {
    const rows = await prisma.$queryRawUnsafe<any[]>(`
      ...
      GROUP BY a.campaign, a.campaign_name, a.advertiser_name, a.budget, a.spent, a.impressions, a.clicks, a.status, a.start_date, a.end_date, a.positions, a.created_at
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    await prisma.$queryRawUnsafe(
      `UPDATE "ads" SET campaign_name=$1, advertiser_name=$2, budget=$3, status=$4, start_date=$5::timestamptz, end_date=$6::timestamptz, positions=$7::text[] WHERE campaign=$8`,
...
router.delete('/:id', async (req, res, next) => {
  try {
    await prisma.$queryRawUnsafe(`DELETE FROM "ads" WHERE campaign=$1`, req.params.id);
    res.json({ success: true });
  } catch(e) { next(e); }
});
